...
public class GraphColoring {
	...
	public int NumberOfColor() {
		...
		while (!result) {
			...
			result = CheckingVertexColored();
		}
		
		return possible_color.size();
	}
	
	public boolean CheckingVertexColored() {
		boolean result = true;
		for (int i = 0; i < graph.nodes.length; i++){
			if (graph.nodes[i].color == ConstantVariable.UNCOLORED){
				result = false;
				break;
			}
		}
	...
	public Node FindTheNextNode(){
		...
		System.out.println(next_saturation_node.get(next_node).value);
